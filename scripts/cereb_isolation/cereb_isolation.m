...
% Do the same for functional images
if isfile(['/data/project/DevCerebToM/suit/input/' subjects{subject} '/' subjects{subject} '_tom_vs_pain_unthresh.nii']) == 0
    display('Image has not been unzipped; unzipping now')
    gunzip(['/data/project/DevCerebToM/suit/input/' subjects{subject} '/' subjects{subject} '_tom_vs_pain_unthresh.nii.gz'])
else
    display('Image is already unzipped; doing nothing')
end
...
% Reslice anatomical images to SUIT space
for subject=1:length(subjects)
    ...
    job.subj(subject).resample = {['/data/project/DevCerebToM/suit/input/' subjects{subject} '/' subjects{subject} '_T1w.nii']};
    job.subj(subject).mask = {['/data/project/DevCerebToM/suit/input/' subjects{subject} '/c_' subjects{subject} '_T1w_pcereb.nii']};
end

suit_reslice_dartel(job);

% Reslice functional images to SUIT space
for subject=1:length(subjects)
    job.subj(subject).affineTr = {['/data/project/DevCerebToM/suit/input/' subjects{subject} '/Affine_' subjects{subject} '_T1w_seg1.mat']}; 
    job.subj(subject).flowfield = {['/data/project/DevCerebToM/suit/input/' subjects{subject} '/u_a_' subjects{subject} '_T1w_seg1.nii']};
    job.subj(subject).resample = {['/data/project/DevCerebToM/suit/input/' subjects{subject} '/' subjects{subject} '_tom_vs_pain_unthresh.nii']};
    job.subj(subject).mask = {['/data/project/DevCerebToM/suit/input/' subjects{subject} '/c_' subjects{subject} '_T1w_pcereb.nii']};
end
