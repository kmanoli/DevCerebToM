...
# Set data directory 
data_dir = r'/data/project/DevCerebToM/adults_ccc'

# Define subjects
sub_list = os.path.join(data_dir, 'adults_ccc.txt')  
...
for sub in subjects:
    ...
    if scan_files:
        ...
        confound_file = glob.glob(os.path.join(data_dir, f'data/{sub}/conf/*confounds_timeseries.tsv'))[0]
        confounds = pd.read_csv(confound_file, skipinitialspace=True, sep='\t')
        filter_col = [col for col in confounds if col.startswith('t_comp')]
        confounds = confounds[filter_col]

        # Define first level model, apply it to scan, and specify the design matrix
        fmri_glm = FirstLevelModel(t_r=0.7, standardize=False, hrf_model='spm + derivative', smoothing_fwhm = 5)
